...
function Scenery() {
  return (
    <div className="flex flex-col gap-10 items-center justify-center py-20 px-20 bg-white">
      ...
      <Tabs defaultValue="Mornings" className="w-full p-4">
        <TabsList className="w-full flex flex-wrap flex-col md:flex-row justify-center gap-2 shadow-xl rounded-md md:rounded-full bg-gray-100 h-max px-4 py-2">
          {["Mornings", "Bonfires", "Trials", "Local Life"].map((tab) => (
            <TabsTrigger
              key={tab}
              value={tab}
              className="p-2 text-sm md:text-base w-full font-semibold rounded-full transition-colors duration-200 hover:bg-gray-200 data-[state=active]:bg-green-600 data-[state=active]:text-white cursor-pointer"
            >
              {tab}
            </TabsTrigger>
          ))}
      ...
      <div className="flex flex-col gap-3">
        ...
        <button className="rounded-full mx-auto px-10 py-3 bg-green-700 text-white font-semibold hover:bg-green-800 cursor-pointer">
          Get In Touch
        </button>
      </div>
    </div>
